...
test('finance navigation is not part of the technical module menu', () => {
  ...
  expect(visibleNavItemsForUser(financeSupremo).map((item) => item.label)).not.toContain('Financeiro');

  expect(canAccessPath(financeCustom, '/m/financeiro')).toBe(false);
  expect(canAccessPath(financeIntermediario, '/m/financeiro/reports')).toBe(false);
  expect(canAccessPath(financeSupremo, '/m/financeiro/reports')).toBe(false);
});
...
test('planning nav item is visible to calendar or cohort operators', () => {
  ...
  expect(visibleNavItemsForUser(user).some((item) => item.to === '/m/tecnico/planejar')).toBe(true);
  expect(canAccessPath(user, '/m/tecnico/planejar')).toBe(true);
});
