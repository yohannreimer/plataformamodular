          ...
          <tbody>
            {ordered.map((row) => (
              <tr key={row.id} className="row-openable" onDoubleClick={() => openCandidateDetail(row)}>
                ...
                <td><StatusBadge value={statusLabel(row.stage)} tone={statusTone(row.stage)} /></td>
                <td><StatusBadge value={statusLabel(row.process_status)} tone={statusTone(row.process_status)} /></td>
