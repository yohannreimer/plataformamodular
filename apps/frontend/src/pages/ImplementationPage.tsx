...
type PortalRealtimeSide = 'cliente' | 'prymeira';
    ...
    try {
      ...
      setConversationRealtime((prev) => ({
        ...
        presence: {
          ...
          prymeira_online: response.presence?.prymeira_online ?? prev.presence.prymeira_online ?? null
        },
        typing: {
    ...
    try {
      ...
      setConversationRealtime((prev) => ({
        ...
        presence: {
          ...
          prymeira_online: snapshot.presence?.prymeira_online ?? prev.presence.prymeira_online ?? null
        },
        typing: {
    ...
    const connect = () => {
      ...
      nextSocket.onmessage = (rawEvent) => {
        ...
        if (payload.type === 'ticket_read') {
          setConversationRealtime((prev) => ({
            ...prev,
            unreadCount: payload.side === 'prymeira' ? 0 : prev.unreadCount,
  ...
  return (
    <div className="page implementation-page">
      ...
      {boardAlerts.length > 0 ? (
        <Panel title={boardMode === 'support' ? 'Alertas de suporte' : 'Alertas de implementação'} className="kanban-config-panel">
          <div className="stack">
      ...
      {!isFiltersCollapsed ? (
        <Panel title="Views e filtros" className="kanban-config-panel">
          <div className="form kanban-config-form">
              ...
              <div className="portal-ticket-conversation-column">
                ...
                >
                  ...
                  {conversationMessages.map((messageItem) => {
                    const authorSide: PortalRealtimeSide = messageItem.author_type === 'Prymeira' ? 'prymeira' : 'cliente';
                    const isOwnMessage = authorSide === 'prymeira';
                    return (
                      <article
                        key={messageItem.id}
                      ...
                      >
                        <div
                          className={`portal-ticket-message-avatar ${authorSide === 'prymeira' ? 'has-logo' : ''}`}
                          aria-hidden="true"
                        >
                          {authorSide === 'prymeira' ? (
