                      ...
                      <tbody>
                        {hoursPending.map((pending) => (
                          <tr key={pending.id}>
                            ...
                            <td><StatusBadge value={statusLabel(pending.status)} tone={statusTone(pending.status)} /></td>
                  ...
                  {moduleHoursInsights.length === 0 ? (
                    ...
                    moduleHoursInsights.map((insight) => (
                      <tr key={`module-hours-${insight.module_id}`}>
                        ...
                        <td><StatusBadge value={statusLabel(insight.status)} tone={statusTone(insight.status)} /></td>
                      </tr>
                    ))
                  )}
                </tbody>
                      ...
                      <li key={moduleItem.module_id} className="timeline-item journey-module-card">
                        <div className="journey-module-head">
                          ...
                          <div className="journey-module-badges">
                            {moduleItem.is_enabled ? <StatusBadge value={statusLabel(moduleItem.status)} tone={statusTone(moduleItem.status)} /> : <span className="chip">Desativado</span>}
                            <span
                  ...
                  <tbody>
                    {data.optionals.map((optionalItem: any) => (
                      <tr key={optionalItem.id}>
                        ...
                        <td><StatusBadge value={statusLabel(optionalItem.status)} tone={statusTone(optionalItem.status)} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {sortedHistory.map((historyItem: any) => (
                      <tr key={historyItem.allocation_id}>
                        ...
                        <td><StatusBadge value={statusLabel(historyItem.status)} tone={statusTone(historyItem.status)} /></td>
                        <td><StatusBadge value={statusLabel(historyItem.cohort_status)} tone={statusTone(historyItem.cohort_status)} /></td>
