...
export function CalendarPage() {
  ...
  return (
    <div className={`page calendar-page${calendarVividMode ? ' calendar-page-vivid' : ''}`}>
      ...
      <Panel title="Visão mensal" className="calendar-month-panel">
                    ...
                    <tbody>
                      {selectedDayEvents.map((event) => (
                        ...
                        >
                          ...
                          <td>
                            <StatusBadge value={statusLabel(event.status)} tone={statusTone(event.status)} />
                            {event.day_start_time && event.day_end_time ? (
                    ...
                    <tbody>
                      {selectedDayActivities.map((activity) => (
                        <tr key={`activity-row-${activity.id}-${activity.calendar_date}`}>
                          ...
                          <td><StatusBadge value={statusLabel(activity.status)} tone={statusTone(activity.status)} /></td>
                          <td>
                ...
                <div className="form-subcard">
                  {!detail ? <p>Clique em uma turma para abrir os detalhes completos.</p> : (
                    <div className="stack">
                      ...
                      <StatusBadge value={statusLabel(detail.status)} tone={statusTone(detail.status)} />
                        ...
                        <tbody>
                          {detail.allocations.map((allocation: any) => (
                            <tr key={allocation.id}>
                              ...
                              <td><StatusBadge value={statusLabel(allocation.status)} tone={statusTone(allocation.status)} /></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
