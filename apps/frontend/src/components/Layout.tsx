...
function topbarContext(pathname: string) {
  const technicalPath = pathname.startsWith(TECHNICAL_BASE_PATH)
    ? pathname.slice(TECHNICAL_BASE_PATH.length) || '/'
    : pathname;
  if (technicalPath.startsWith('/planejar')) {
    return {
  ...
  if (technicalPath.startsWith('/calendario')) {
    return {
  ...
  if (technicalPath.startsWith('/turmas')) {
    return {
  ...
  if (technicalPath.startsWith('/clientes')) {
    return {
  ...
  if (technicalPath.startsWith('/tecnicos')) {
    return {
  ...
  if (technicalPath.startsWith('/admin')) {
    return {
...
export function Layout({ children, loggedUser, navItems, onLogout }: LayoutProps) {
  ...
  const isPlanningRoute = location.pathname.startsWith(`${TECHNICAL_BASE_PATH}/planejar`);
