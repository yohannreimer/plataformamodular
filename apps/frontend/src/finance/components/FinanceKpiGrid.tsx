...
import { financePath } from '../routes';
...
function kpiHref(kpi: FinanceExecutiveKpi) {
  ...
  if (key === 'receber') return financePath('receivables');
  if (key === 'pagar') return financePath('payables');
  if (key === 'faturamento') return financePath('transactions?kind=income');
  if (key === 'despesa') return financePath('transactions?kind=expense');
  if (key === 'resultado') return financePath('transactions');
  if (key === 'atras') return financePath('receivables?status=overdue');
  if (key === 'concilia') return financePath('reconciliation');
  return null;
}
