...
export function FinanceQueuePanel({ items, currency, loading = false }: FinanceQueuePanelProps) {
  ...
  const emptyRows = [
    { id: 'empty-1', status: 'Crítico', title: 'Atraso a receber', detail: 'Sem títulos críticos no momento.', amount_cents: 0, tone: 'critical' as const, href: financePath('receivables'), cta: 'Ver recebíveis' },
    { id: 'empty-2', status: 'Hoje', title: 'Vencem hoje', detail: 'Não há pagamentos vencendo nesta janela.', amount_cents: 0, tone: 'warning' as const, href: financePath('payables'), cta: 'Ver pagamentos' },
    { id: 'empty-3', status: 'Fila', title: 'Pendências', detail: 'Sem lançamentos aguardando match.', amount_cents: 0, tone: 'warning' as const, href: financePath('reconciliation'), cta: 'Conciliar' },
    { id: 'empty-4', status: 'Próximo', title: 'A pagar em breve', detail: 'Nenhuma obrigação próxima da data crítica.', amount_cents: 0, tone: 'neutral' as const, href: financePath('payables'), cta: 'Ver obrigações' }
  ];
      ...
      <div className="finance-queue-panel__content">
        {items.map((item) => (
          <article key={item.id} className={`finance-queue-item finance-queue-item--${item.tone}`}>
            ...
            <h3>{item.title}</h3>
            <p>{item.detail}</p>
            <Link to={normalizeFinancePath(item.href)} className="finance-queue-item__link finance-queue-item__link--flat">
