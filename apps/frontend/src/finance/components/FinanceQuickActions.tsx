...
export function FinanceQuickActions({ actions, loading = false }: FinanceQuickActionsProps) {
  const loadingCards = [
    { id: 'loading-1', label: 'Novo lançamento', detail: 'Carregando atalhos do financeiro.', href: financePath('transactions') },
    { id: 'loading-2', label: 'Registrar recebível', detail: 'Carregando atalhos do financeiro.', href: financePath('receivables') },
    { id: 'loading-3', label: 'Registrar obrigação', detail: 'Carregando atalhos do financeiro.', href: financePath('payables') },
    { id: 'loading-4', label: 'Conciliar extrato', detail: 'Carregando atalhos do financeiro.', href: financePath('reconciliation') }
  ];
  ...
  const canonicalCards = [
    { id: 'canonical-1', label: 'Novo lançamento', detail: 'Registrar entrada ou saída.', href: financePath('transactions') },
    { id: 'canonical-2', label: 'Registrar recebível', detail: 'Nova conta a receber.', href: financePath('receivables') },
    { id: 'canonical-3', label: 'Registrar obrigação', detail: 'Nova conta a pagar.', href: financePath('payables') },
    { id: 'canonical-4', label: 'Conciliar extrato', detail: 'Processar pendências bancárias.', href: financePath('reconciliation') }
  ];
  ...
  return (
    ...
    >
      <div className="finance-quick-actions-panel__content">
        {[...actions, ...canonicalCards.filter((canonical) => !actions.some((action) => normalizeFinancePath(action.href) === canonical.href))].slice(0, 4).map((action) => (
          <Link key={action.id} to={normalizeFinancePath(action.href)} className="finance-quick-action finance-quick-action--flat">
            <strong>{action.label}</strong>
            <span>{action.detail}</span>
          </Link>
        ))}
