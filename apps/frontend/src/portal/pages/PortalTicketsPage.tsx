...
import prymeiraSeloIcon from '../../assets/prymeira-selo.svg';
import type {
...
function sourceLabel(source: PortalTicket['source']) {
  return source === 'Portal' ? 'Cliente via portal' : 'Operação Prymeira';
}
...
function threadRealtimeFromResponse(response: PortalTicketThreadResponse): ThreadRealtimeState {
  ...
  return {
    ...
    lastReadAt: response.last_read_at ?? response.last_read_cliente_at ?? response.last_read_prymeira_at ?? null,
    presence: {
      client_online: response.presence?.client_online ?? null,
      prymeira_online: response.presence?.prymeira_online ?? null
    },
    typing: {
...
function mergeThreadRealtimeFromResponse(
  ...
  return {
    ...
    presence: {
      ...
      prymeira_online: response.presence?.prymeira_online ?? previous.presence.prymeira_online ?? null
    },
    typing: {
...
export function PortalTicketsPage({ api, isInternal, sessionToken }: PortalTicketsPageProps) {
        ...
        return nextItems.map((item) => {
          ...
          return {
            ...
            realtime: {
              ...
              prymeira_online: realtime?.prymeira_online ?? previous?.realtime?.prymeira_online ?? null,
  ...
  const viewerSide: PortalRealtimeSide = isInternal ? 'prymeira' : 'cliente';
  const counterpartLabel = viewerSide === 'prymeira' ? 'Cliente' : 'Equipe Prymeira';
  const counterpartPresence = viewerSide === 'prymeira'
    ? threadRealtime.presence.client_online
    : threadRealtime.presence.prymeira_online;
        ...
        item.id !== ticketId
          ...
          : {
            ...
            realtime: {
              ...
              prymeira_online: snapshot.presence?.prymeira_online ?? item.realtime?.prymeira_online ?? null,
      ...
      if (selectedTicketIdRef.current === ticketId) {
        setThreadRealtime((prev) => ({
          ...
          presence: {
            ...
            prymeira_online: snapshot.presence?.prymeira_online ?? prev.presence.prymeira_online
          },
          typing: {
            ...
            item.id !== payload.ticket_id
              ...
              : {
                ...
                realtime: {
                  ...
                  prymeira_online: payload.side === 'prymeira' ? payload.online : item.realtime?.prymeira_online ?? null
                }
              }
          )));
          if (selectedTicketIdRef.current === payload.ticket_id) {
            setThreadRealtime((prev) => ({
              ...
              presence: {
                ...
                prymeira_online: payload.side === 'prymeira' ? payload.online : prev.presence.prymeira_online
              }
            }));
          }
          return;
            ...
            <div className="portal-ticket-list portal-ticket-list-premium">
              {sortedItems.length === 0 ? (
                <div className="portal-empty-state portal-support-empty-state">
                  ...
                  <p>Use “Nova solicitação” para registrar a primeira demanda ao time Prymeira.</p>
                </div>
              ) : null}
              ...
              {sortedItems.map((item) => {
                ...
                const itemCounterpartOnline = viewerSide === 'prymeira'
                  ? item.realtime?.client_online
                  : item.realtime?.prymeira_online;
                ...
                >
                  ...
                  {!threadLoading && threadMessages.length === 0 ? (
                    <div className="portal-empty-state portal-support-empty-state">
                      ...
                      <p>Use o composer abaixo para iniciar esta conversa com o time Prymeira.</p>
                    </div>
                  ) : null}
                  {threadMessages.map((message) => {
                    const messageSide: PortalRealtimeSide = message.author_type === 'Prymeira' ? 'prymeira' : 'cliente';
                    const isOwnMessage = messageSide === viewerSide;
                    return (
                      <article
                        key={message.id}
                        className={`portal-ticket-message portal-ticket-message-premium ${isOwnMessage ? 'is-client' : 'is-prymeira'}`}
                      >
                        <div
                          className={`portal-ticket-message-avatar ${messageSide === 'prymeira' ? 'has-logo' : ''}`}
                          aria-hidden="true"
                        >
                          {messageSide === 'prymeira' ? (
                            <img src={prymeiraSeloIcon} alt="" className="portal-ticket-message-avatar-logo" />
                          ) : 'C'}
                        </div>
