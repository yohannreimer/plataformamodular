    ...
    return (
      <article key={item.id} className={`portal-agenda-item ${agendaStatusTone(item.status)}`}>
        <div className="portal-agenda-main">
          ...
          {item.source === 'manual' ? (
            <small className="form-hint">Evento manual ajustado pela equipe Prymeira.</small>
          ) : null}
        </div>
  ...
  return (
    <section className="portal-panel">
      <header className="portal-panel-header">
        <h2>Agenda</h2>
        <p>Acompanhe próximos eventos e histórico concluído da sua operação com a Prymeira.</p>
      </header>
